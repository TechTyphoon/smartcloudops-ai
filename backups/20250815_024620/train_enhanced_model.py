...
def main():
    ...
    try:
        ...
        if len(data) < min_data_points:
            logger.warning(f"Limited data points ({len(data)}), but proceeding with training")
        ...
        if training_result['status'] == 'success':
            ...
            print(f"🔄 Iterations Trained: {training_result.get('iterations_trained', 1)}")
            print(f"🏆 Model Quality: {training_result.get('model_quality', 'standard')}")
            ...
            if detector.save_model():
                ...
                print(f"   Anomalies detected: {sum(1 for r in test_result if r['is_anomaly'])}")
            ...
            logger.error(f"Enhanced model training failed: {training_result.get('reason', 'Unknown error')}")
            return False
