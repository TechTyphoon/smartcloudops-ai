...
class ContinuousHealthMonitor:
    def __init__(self):
        ...
        self.running = True
        self.health_data = []
        self.s3_bucket = f"smartcloudops-uptime-logs-{datetime.now().strftime('%Y%m%d')}"
        ...
        self.endpoints = {
            ...
            "/anomaly/status": {"method": "GET", "expected_status": 200, "critical": True},
            "/anomaly/batch": {"method": "POST", "expected_status": 200, "critical": True,
                              "payload": {"metrics": [{"cpu": 75.2}, {"memory": 45.1}]}},
            "/anomaly/train": {"method": "POST", "expected_status": 200, "critical": False,
                              "payload": {"type": "incremental"}},
            
            # Remediation endpoints
            "/remediation/status": {"method": "GET", "expected_status": 200, "critical": True},
            "/remediation/execute": {"method": "POST", "expected_status": 200, "critical": True,
                                   "payload": {"action": "restart_service", "target": "test", "dry_run": True}},
            "/remediation/evaluate": {"method": "POST", "expected_status": 200, "critical": False,
                                    "payload": {"remediation_id": "test_rem_123"}},
            
            # ChatOps endpoints
            "/chatops/history": {"method": "GET", "expected_status": 200, "critical": True},
            "/chatops/context": {"method": "GET", "expected_status": 200, "critical": True},
            "/chatops/analyze": {"method": "POST", "expected_status": 200, "critical": True,
    ...
    def run_health_check(self):
        ...
        critical_failures = sum(1 for r in check_results if not r["success"] and r["critical"])
        ...
        logger.info(f"{status_emoji} Health Check Summary: {successful_checks}/{total_checks} endpoints healthy "
    ...
    def auto_fix_failures(self, health_summary):
        ...
        critical_failures = [r for r in health_summary["detailed_results"] 
                           if not r["success"] and r["critical"]]
        
        if not critical_failures:
            return
        
        logger.warning(f"Attempting auto-fix for {len(critical_failures)} critical failures...")
        ...
        for failure in critical_failures:
            ...
            if retry_result["success"]:
                ...
                if endpoint == "/health":
                    logger.critical("❗ Health endpoint is failing - service may be down!")
    ...
    def get_health_report(self):
        ...
        avg_success_rate = sum(check["success_rate"] for check in recent_checks) / len(recent_checks)
        total_critical_failures = sum(check["critical_failures"] for check in recent_checks)
    ...
    try:
        while True:
            ...
            if monitor.health_data:
                ...
                logger.info(f"📊 Status Update: {latest['success_rate']}% success rate, "
