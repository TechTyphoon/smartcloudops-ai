# Optional CodeDeploy Blue/Green wiring for ECS

locals {
  blue_green_enabled = var.enable_blue_green
}

resource "aws_iam_role" "codedeploy_role" {
  count = local.blue_green_enabled ? 1 : 0
  name  = "${var.project_name}-codedeploy-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17",
    Statement = [{
      Effect = "Allow",
      Principal = { Service = "codedeploy.amazonaws.com" },
      Action   = "sts:AssumeRole"
    }]
  })
}

resource "aws_iam_role_policy_attachment" "codedeploy_role_attach" {
  count      = local.blue_green_enabled ? 1 : 0
  role       = aws_iam_role.codedeploy_role[0].name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSCodeDeployRoleForECS"
}

resource "aws_codedeploy_app" "ecs_app" {
  count        = local.blue_green_enabled ? 1 : 0
  name         = "${var.project_name}-cd-app"
  compute_platform = "ECS"
}

resource "aws_codedeploy_deployment_group" "ecs_dg" {
  count                  = local.blue_green_enabled ? 1 : 0
  app_name               = aws_codedeploy_app.ecs_app[0].name
  deployment_group_name  = "${var.project_name}-cd-dg"
  service_role_arn       = aws_iam_role.codedeploy_role[0].arn
  deployment_config_name = "CodeDeployDefault.ECSAllAtOnce"

  blue_green_deployment_config {
    terminate_blue_instances_on_deployment_success {
      action                           = "TERMINATE"
      termination_wait_time_in_minutes = 5
    }
    deployment_ready_option {
      action_on_timeout = "CONTINUE_DEPLOYMENT"
    }
  }

  ecs_service {
    cluster_name = aws_ecs_cluster.app_cluster.name
    service_name = aws_ecs_service.app_service.name
  }

  load_balancer_info {
    target_group_pair_info {
      prod_traffic_route {
        listener_arns = length(var.domain_name) > 0 ? [aws_lb_listener.https[0].arn] : [aws_lb_listener.http_plain[0].arn]
      }
      target_group {
        name = aws_lb_target_group.app_tg.name
      }
      target_group {
        name = aws_lb_target_group.app_tg_green.name
      }
    }
  }
}

