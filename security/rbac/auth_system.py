...
class UserManager:
    ...
    def create_user(self, username: str, email: str, password: str, role: UserRole) -> User:
        ...
        with self._get_db_connection() as conn:
            ...
            try:
                ...
                # Log user creation
                self._log_audit_event(user.id, "user_created", "users", {"username": username, "role": role.value})
    ...
    def authenticate_user(self, username: str, password: str, ip_address: str = None) -> Optional[AccessToken]:
        """Authenticate user and return access token"""
        user = self.get_user_by_username(username)
        
        if not user:
            self._log_audit_event(None, "login_failed", "auth", {"username": username, "reason": "user_not_found"}, ip_address, False)
            return None
        
        # Check if user is locked
        if user.locked_until and datetime.now() < user.locked_until:
            self._log_audit_event(user.id, "login_failed", "auth", {"reason": "account_locked"}, ip_address, False)
            return None
        
        # Check if user is active
        if not user.is_active:
            self._log_audit_event(user.id, "login_failed", "auth", {"reason": "account_disabled"}, ip_address, False)
            return None
        ...
        if not bcrypt.checkpw(password.encode('utf-8'), user.password_hash.encode('utf-8')):
            ...
            self._log_audit_event(user.id, "login_failed", "auth", {"reason": "invalid_password"}, ip_address, False)
            return None
        ...
        self._log_audit_event(user.id, "login_success", "auth", {"token_id": token.token_id}, ip_address, True)
    ...
    def revoke_token(self, token_id: str, user_id: str = None):
        ...
        with self._get_db_connection() as conn:
            ...
            self._log_audit_event(user_id, "token_revoked", "auth", {"token_id": token_id})
    ...
    def _generate_access_token(self, user: User, ip_address: str = None) -> AccessToken:
        ...
        # Create JWT token
        token_string = jwt.encode(access_token.to_dict(), self.jwt_secret, algorithm='HS256')
    ...
    def _log_audit_event(self, user_id: str, action: str, resource: str, details: Dict[str, Any] = None, 
        ...
        with self._get_db_connection() as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (user_id, action, resource, json.dumps(details) if details else None, ip_address, success))
            conn.commit()
