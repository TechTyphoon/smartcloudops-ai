...
class MLTrainingData(BaseModel):
    ...
    def __repr__(self):
        return f'<MLTrainingData {self.id}: {self.anomaly_label} - CPU={self.cpu_usage}%>'
...
class AnomalyDetection(BaseModel):
    ...
    system_metrics_id = db.Column(db.Integer, db.ForeignKey('system_metrics.id'), nullable=True)
    system_metrics = relationship('SystemMetrics', backref='anomalies')
    
    # Detection details
    description = db.Column(db.Text, nullable=False)
    affected_components = db.Column(db.JSON, nullable=True)  # Store list of affected system components
    recommended_actions = db.Column(db.JSON, nullable=True)  # Store list of recommended remediation actions
    
    # Status tracking
    status = db.Column(db.String(50), nullable=False, default='detected')  # detected, investigating, resolved, false_positive
...
class RemediationAction(BaseModel):
    ...
    # Action metadata
    anomaly_id = db.Column(db.Integer, db.ForeignKey('anomaly_detections.id'), nullable=False)
    anomaly = relationship('AnomalyDetection', backref='remediation_actions')
    
    # Action details
    action_type = db.Column(db.String(100), nullable=False)  # restart_service, clear_cache, scale_resources, etc.
...
class ChatOpsInteraction(BaseModel):
    ...
    # Analysis results
    analysis_result = db.Column(db.JSON, nullable=False)  # Store complex analysis results
    ...
    user_feedback = db.Column(db.String(20), nullable=True)  # helpful, not_helpful, etc.
    
    def __repr__(self):
        return f'<ChatOpsInteraction {self.id}: {self.intent} - {self.confidence_score}>'
...
class HealthCheck(BaseModel):
    ...
    # Check metadata
    timestamp = db.Column(db.DateTime, nullable=False, default=datetime.utcnow, index=True)
    check_type = db.Column(db.String(50), nullable=False)  # database, api, system, external
...
class SecurityScan(BaseModel):
    ...
    # Scan metadata
    timestamp = db.Column(db.DateTime, nullable=False, default=datetime.utcnow, index=True)
    scan_type = db.Column(db.String(50), nullable=False)  # bandit, dependency, configuration
    ...
    def __repr__(self):
        return f'<SecurityScan {self.id}: {self.scan_type} - {self.total_issues} issues>'
