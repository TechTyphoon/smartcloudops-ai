...
class ProductionConfig:
    ...
    def __init__(self, environment: Environment = Environment.PRODUCTION):
        ...
        self.config_file = Path(
            f"deployment/production/config_{environment.value}.json"
        )
        ...
        self.security = SecurityConfig(
            ...
            max_requests_per_minute=int(
                self._get_env("MAX_REQUESTS_PER_MINUTE", "100")
            ),
            cors_origins=self._get_env_list("CORS_ORIGINS", ["http://localhost:3000"]),
            ssl_enabled=self._get_env_bool(
                "SSL_ENABLED", environment == Environment.PRODUCTION
            ),
    ...
    def _get_or_generate_secret(self, key: str) -> str:
        ...
        logger.warning(
            f"Generated new secret for {key}. Set {key} environment variable in production!"
        )
        return secret
    ...
    def validate_config(self) -> List[str]:
        ...
        if self.security.ssl_enabled:
            if (
                ...
                or not Path(self.security.ssl_cert_path).exists()
            ):
                issues.append("SSL certificate file not found")

            if (
                not self.security.ssl_key_path
                or not Path(self.security.ssl_key_path).exists()
            ):
