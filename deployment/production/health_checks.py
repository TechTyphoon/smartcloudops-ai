...
class BaseHealthCheck:
    ...
    def _create_result(self, status: HealthStatus, message: str, 
                      response_time: float, details: Dict[str, Any] = None) -> HealthCheckResult:
...
class DatabaseHealthCheck(BaseHealthCheck):
    ...
    async def check(self) -> HealthCheckResult:
        ...
        try:
            ...
            cursor.execute("CREATE TABLE IF NOT EXISTS health_check_test (id INTEGER, timestamp TEXT)")
            cursor.execute("INSERT INTO health_check_test (id, timestamp) VALUES (?, ?)", 
...
class MLOpsServiceCheck(BaseHealthCheck):
    ...
    # Test basic operations
            ...
            core_components = [experiments_available, models_available, data_pipeline_available]
...
class HealthMonitor:
    ...
    async def run_all_checks(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'healthy_checks': sum(1 for s in component_statuses.values() if s == HealthStatus.HEALTHY),
            'warning_checks': sum(1 for s in component_statuses.values() if s == HealthStatus.WARNING),
            'critical_checks': sum(1 for s in component_statuses.values() if s == HealthStatus.CRITICAL),
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        if recent_reports:
            healthy_reports = sum(1 for r in recent_reports if r['overall_status'] == 'healthy')
