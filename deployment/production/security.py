...
class RateLimiter:
    ...
    def is_allowed(self, identifier: str, rule: str = 'default') -> bool:
        ...
        blocks_in_hour = sum(1 for t in self.blocked_ips[identifier] 
                           if current_time - t < 3600)
        
        if blocks_in_hour >= 3:  # 3 violations in an hour = persistent block
            self.persistent_blocks.add(identifier)
            logger.warning(f"IP {identifier} persistently blocked for repeated violations")
...
class SecurityAuditor:
    ...
    audit_results['checks'].append(result)
        ...
        if audit_results['max_score'] > 0:
            score_percentage = (audit_results['overall_score'] / audit_results['max_score']) * 100
        else:
            score_percentage = 0
        ...
        for file_path in critical_files:
            ...
            if path.exists():
                ...
                if path.is_file() and mode in ['777', '666']:
                    ...
                    issues.append(f"Directory {file_path} has overly permissive permissions: {mode}")
                else:
                    score += 2
    ...
    def _check_secret_exposure(self) -> Dict[str, Any]:
        ...
        if not issues:
            ...
            recommendations.append("Move secrets to environment variables or secure config")
    ...
    def _check_cors_configuration(self) -> Dict[str, Any]:
        ...
        if cors_origins:
            ...
            if '*' in origins:
                ...
                if env == 'production' and any('localhost' in origin for origin in origins):
                ...
                for field, validator_config in validators.items():
                    if field in data:
                        ...
                        if 'max_length' in validator_config:
                            value = SecurityValidator.sanitize_input(value, validator_config['max_length'])
                            data[field] = value
                        
                        if 'required' in validator_config and validator_config['required']:
