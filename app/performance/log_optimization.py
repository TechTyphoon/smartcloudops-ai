    ...
    def _compress_file(self, file_path: Path):
        ...
        try:
            ...
            with open(file_path, "rb") as f_in:
                with gzip.open(
                    compressed_path, "wb", compresslevel=self.config.compression_level
                ) as f_out:
            ...
            if not self.running:
                ...
                for i in range(self.config.async_workers):
                    thread = threading.Thread(
                        target=self._worker_loop, daemon=True, name=f"log-worker-{i}"
                    )
                    thread.start()
...
class LogManager:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "current_file": (
                str(self.rotator.current_file) if self.rotator.current_file else None
            ),
            "current_size": self.rotator.current_size,
            "queue_size": (
                self.async_writer.message_queue.qsize()
