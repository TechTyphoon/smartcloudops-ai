...
class AIHandler:
    ...
    def process_query(self, query: str) -> Dict[str, Any]:
        ...
        if any(word in query_lower for word in ["status", "health", "operational", "running"]):
            response_type = "system_status"
        elif any(word in query_lower for word in ["anomaly", "anomalies", "detect", "issue", "problem"]):
            response_type = "anomaly_detection"
        elif any(word in query_lower for word in ["performance", "optimize", "speed", "fast"]):
            response_type = "performance_optimization"
        elif any(word in query_lower for word in ["security", "secure", "threat", "vulnerability"]):
            response_type = "security_analysis"
        else:
            response_type = "system_status"
        
        # Get random response from appropriate category
        responses = self.demo_responses.get(response_type, self.demo_responses["system_status"])
    ...
    def _generate_suggestions(self, response_type: str) -> list:
        ...
        return suggestion_sets.get(response_type, [
