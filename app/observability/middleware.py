...
from .tracing import get_trace_id, get_span_id, create_span
...
class RequestIDWSGIHandler(WSGIRequestHandler):
    ...
    def log_request(self, code='-', size='-'):
        ...
        corr_id = getattr(g, 'correlation_id', 'unknown') if has_request_context() else 'no-context'
        
        # Format: IP - - [timestamp] "METHOD path HTTP/version" status size "correlation_id"
        self.log('info', '"%s" %s %s [%s]',
                self.requestline, str(code), str(size), corr_id)
