...
def create_span(
    ...
    span_kwargs = {}
    if kind:
        span_kwargs['kind'] = getattr(trace.SpanKind, kind.upper(), trace.SpanKind.INTERNAL)
            ...
            try:
                ...
                if isinstance(result, dict):
                    if 'anomalies_found' in result:
                        span.set_attribute("anomaly.count", len(result['anomalies_found']))
                    if 'severity' in result:
                        span.set_attribute("anomaly.max_severity", result['severity'])
                    if 'confidence' in result:
                        span.set_attribute("anomaly.confidence", result['confidence'])
                
                span.set_attribute("anomaly.detection_duration_ms", (time.time() - start_time) * 1000)
            ...
            try:
                ...
                if isinstance(result, dict):
                    ...
                    if 'target_resources' in result:
                        span.set_attribute("remediation.target_count", len(result['target_resources']))
                
                span.set_attribute("remediation.duration_ms", (time.time() - start_time) * 1000)
                span.set_status(trace.Status(trace.StatusCode.OK))
                ...
                try:
                    ...
                    if isinstance(result, dict):
                        ...
                        if 'predictions' in result:
                            span.set_attribute("ml.prediction_count", len(result['predictions']))
                    
                    span.set_attribute("ml.duration_ms", (time.time() - start_time) * 1000)
                    span.set_status(trace.Status(trace.StatusCode.OK))
