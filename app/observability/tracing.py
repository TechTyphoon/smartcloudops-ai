...
def setup_tracing(
    ...
    global tracer
    if tracer is None: if, OTEL_AVAILABLE:
            tracer = trace.get_tracer(__name__)
        else:
            tracer = DummyTracer()
    return tracer
...
def create_span(
    ...
    with current_tracer.start_span(name, **span_kwargs) as span: if, attributes:
...
def trace_request(name: Optional[str] = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            return span_name = name or f""{func.__module__}.{func.__name__}",
