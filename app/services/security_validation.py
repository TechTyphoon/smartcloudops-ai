...
class SecurityValidation:
    ...
    def __init__(self):
        ...
        self.suspicious_patterns = [
            ...
            "import os", "import sys", "subprocess", "os.system", "__import__"
        ]
        ...
        if isinstance(data, (dict, list)):
            ...
            if isinstance(data, dict):
                for value in data.values():
                    max_depth = max(max_depth, self._get_nesting_depth(value, current_depth + 1))
            else:  # list
                for item in data:
                    max_depth = max(max_depth, self._get_nesting_depth(item, current_depth + 1))
            return max_depth
        else:
            return current_depth
