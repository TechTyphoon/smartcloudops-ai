...
class RemediationService:
    ...
    def get_remediation_actions(
        ...
        if action_type:
            filtered_actions = [r for r in filtered_actions if r["action_type"] == action_type]
        if priority:
            filtered_actions = [r for r in filtered_actions if r["priority"] == priority]
        if anomaly_id:
            filtered_actions = [r for r in filtered_actions if r["anomaly_id"] == anomaly_id]
    ...
    def create_remediation_action(self, action_data: Dict) -> Dict:
        ...
        valid_action_types = ["scale_up", "scale_down", "restart_service", "cleanup_disk", "custom"]
        if action_data["action_type"] not in valid_action_types:
            raise ValueError(f"Invalid action_type. Must be one of: {', '.join(valid_action_types)}")
    ...
    def update_remediation_action(self, action_id: int, update_data: Dict) -> Optional[Dict]:
        """
        ...
        updateable_fields = ["action_name", "description", "status", "priority", "parameters", "execution_result", "error_message"]
    ...
    def get_remediation_statistics(self) -> Dict:
        ...
        success_rate = (completed_actions / total_executed * 100) if total_executed > 0 else 0
