        ...
        with open(env_path, "r") as f:
                ...
                if line and not line.startswith("#") and "=" in line:
                    key, value = line.split("=", 1)
                        ...
                        if key in ["JWT_SECRET_KEY", "DATABASE_PASSWORD", "REDIS_PASSWORD", "OPENAI_API_KEY"]:
                            if len(value) < 16:
...
class Config:
    ...
    @classmethod
    def _validate_security_config(cls, config_dict: Dict[str, Any], errors: List[str]) -> None:
        """Validate security-related configuration."""
        # Validate secret keys
        if not config_dict.get("secret_key") or len(config_dict["secret_key"]) < 32:
            errors.append("SECRET_KEY must be at least 32 characters long")

        if not config_dict.get("jwt_secret_key") or len(config_dict["jwt_secret_key"]) < 32:
    ...
    @classmethod
    def _validate_database_config(cls, config_dict: Dict[str, Any], errors: List[str]) -> None:
        """Validate database configuration."""
        database_url = config_dict.get("database_url", "")
        if database_url:
            # Validate database URL format
            if not database_url.startswith(("postgresql://", "postgresql+psycopg://", "mysql://", "sqlite://")):
    ...
    @classmethod
    def _validate_redis_config(cls, config_dict: Dict[str, Any], errors: List[str]) -> None:
    ...
    @classmethod
    def _validate_ai_config(cls, config_dict: Dict[str, Any], errors: List[str]) -> None:
    ...
    @classmethod
    def _validate_numeric_values(cls, config_dict: Dict[str, Any], errors: List[str]) -> None:
    ...
    @classmethod
    def _validate_cors_config(cls, config_dict: Dict[str, Any], errors: List[str]) -> None:
    ...
    def from_env(cls) -> "Config":
        ...
        # Security configuration
        config_dict["jwt_access_token_expiry_hours"] = int(os.getenv("JWT_ACCESS_TOKEN_EXPIRY_HOURS", "24"))
        config_dict["jwt_refresh_token_expiry_days"] = int(os.getenv("JWT_REFRESH_TOKEN_EXPIRY_DAYS", "7"))
        ...
        config_dict["lockout_duration_minutes"] = int(os.getenv("LOCKOUT_DURATION_MINUTES", "15"))

        # CORS configuration
        cors_origins = os.getenv("CORS_ORIGINS", "http://localhost:3000,http://127.0.0.1:3000")
        config_dict["cors_origins"] = [origin.strip() for origin in cors_origins.split(",") if origin.strip()]
        config_dict["cors_supports_credentials"] = os.getenv("CORS_SUPPORTS_CREDENTIALS", "true").lower() == "true"

        # Database configuration
        config_dict["database_url"] = os.getenv("DATABASE_URL", "")
        config_dict["database_pool_size"] = int(os.getenv("DATABASE_POOL_SIZE", "20"))
        config_dict["database_max_overflow"] = int(os.getenv("DATABASE_MAX_OVERFLOW", "30"))
        config_dict["database_pool_timeout"] = int(os.getenv("DATABASE_POOL_TIMEOUT", "30"))
        config_dict["database_pool_recycle"] = int(os.getenv("DATABASE_POOL_RECYCLE", "3600"))
        ...
        config_dict["openai_temperature"] = float(os.getenv("OPENAI_TEMPERATURE", "0.3"))
        ...
        config_dict["gemini_temperature"] = float(os.getenv("GEMINI_TEMPERATURE", "0.3"))

        # Enhanced ML Features
        config_dict["enable_enhanced_anomaly"] = os.getenv("ENABLE_ENHANCED_ANOMALY", "false").lower() == "true"
        config_dict["enable_multi_metric_correlation"] = os.getenv("ENABLE_MULTI_METRIC_CORRELATION", "false").lower() == "true"
        config_dict["enable_failure_prediction"] = os.getenv("ENABLE_FAILURE_PREDICTION", "false").lower() == "true"
        config_dict["enable_anomaly_explanation"] = os.getenv("ENABLE_ANOMALY_EXPLANATION", "false").lower() == "true"
        ...
        config_dict["log_max_size"] = int(os.getenv("LOG_MAX_SIZE", str(100 * 1024 * 1024)))
        config_dict["log_backup_count"] = int(os.getenv("LOG_BACKUP_COUNT", "5"))

        # Remediation configuration
        config_dict["require_approval"] = os.getenv("REQUIRE_APPROVAL", "false").lower() == "true"
        config_dict["max_actions_per_hour"] = int(os.getenv("MAX_ACTIONS_PER_HOUR", "10"))
        ...
        config_dict["remediation_tag_value"] = os.getenv("REMEDIATION_TAG_VALUE", "smartcloudops-ai-application")
        config_dict["ssm_service_name"] = os.getenv("SSM_SERVICE_NAME", "smartcloudops-app")
        ...
        # Security headers configuration
        config_dict["security_headers_enabled"] = os.getenv("SECURITY_HEADERS_ENABLED", "true").lower() == "true"
        config_dict["content_security_policy"] = os.getenv("CONTENT_SECURITY_POLICY", "")
        config_dict["x_frame_options"] = os.getenv("X_FRAME_OPTIONS", "DENY")
        config_dict["x_content_type_options"] = os.getenv("X_CONTENT_TYPE_OPTIONS", "nosniff")

        # Rate limiting
        config_dict["rate_limit_enabled"] = os.getenv("RATE_LIMIT_ENABLED", "true").lower() == "true"
        config_dict["rate_limit_requests_per_minute"] = int(os.getenv("RATE_LIMIT_REQUESTS_PER_MINUTE", "100"))
        config_dict["rate_limit_requests_per_hour"] = int(os.getenv("RATE_LIMIT_REQUESTS_PER_HOUR", "1000"))

        # Cache configuration
        config_dict["cache_enabled"] = os.getenv("CACHE_ENABLED", "true").lower() == "true"
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        config_dict = {}
        sensitive_keys = ["secret_key", "jwt_secret_key", "database_url", "redis_password", 
                         "openai_api_key", "gemini_api_key", "aws_access_key_id", "aws_secret_access_key"]
    ...
    def get_database_url(self) -> Optional[str]:
        ...
        try:
            ...
            if not self.DATABASE_URL.startswith(("postgresql://", "postgresql+psycopg://", "mysql://", "sqlite://")):
    ...
    def is_production(self) -> bool:
        ...
        return not self.DEBUG and os.getenv("FLASK_ENV", "development").lower() == "production"
