...
class RemediationEngine:
    ...
    def __init__(self, rules_path: str = "configs/remediation-rules.yaml"):
        ...
        self.require_approval: bool = os.getenv("REQUIRE_APPROVAL", "false").lower() == "true"
    ...
    def handle_anomaly_async(self, anomaly_result: Dict[str, Any]) -> None:
        threading.Thread(target=self._handle_anomaly, args=(anomaly_result,), daemon=True).start()
    ...
    def _handle_anomaly(self, anomaly_result: Dict[str, Any]) -> None:
        try:
            ...
            threshold = float(self.rules.get("anomaly", {}).get("severity_threshold", 0.7))
            if severity < threshold:
                return

            action = self.rules.get("anomaly", {}).get("action", "restart_service")
            if action == "restart_service":
                self._execute_restart_service(reason=f"Anomaly severity {severity} >= {threshold}")
    ...
    def _execute_restart_service(self, reason: str) -> None:
        ...
        if not allowed:
            ...
            self.notifier.send_slack_message(
                ...
                message=why,
                fields={"action": "restart_service", "resource": resource_id, "reason": reason},
            )
            return
        ...
        if result.get("status") == "success":
            ...
            self.notifier.send_slack_message(
                ...
                message=reason,
                fields={"instances": ",".join(result.get("instances", [])), "service": self.service_name},
            )
        else:
            self.notifier.send_slack_message(
