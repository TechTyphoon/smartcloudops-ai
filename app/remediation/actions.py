...
class ActionManager:
    ...
    def execute_action(self, action: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Executing action: {action_type} on {target} with priority {priority}")
            ...
            logger.info(f"Action {action_type} completed with status: {result.get('status', 'unknown')}")
            return result
            
        except Exception as e:
            logger.error(f"Error executing action {action.get('action', 'unknown')}: {e}")
            return {
                'status': 'error',
                'error': str(e),
                'action_type': action.get('action', 'unknown'),
    ...
    def _restart_service(self, target: str, action: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if not instances:
                return {'status': 'error', 'error': f'No instances found for target: {target}'}
            ...
            for instance_id in instances:
                try:
                    ...
                    logger.error(f"Error restarting service on instance {instance_id}: {e}")
                    results.append({
                        'instance_id': instance_id,
                        'error': str(e)
                    })
...
'''
    ...
    def _wait_for_command_completion(self, command_id: str, instance_id: str, timeout: int = 300) -> Dict[str, Any]:
