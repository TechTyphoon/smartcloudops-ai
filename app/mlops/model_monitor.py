...
class ModelMonitor:
    ...
    def __init__(self, monitor_path: str = "ml_models/monitoring", model_registry=None):
        return self.monitor_path = Path(monitor_path)
    ...
    def start_monitoring()
        self, model_id: str, model_version: str, monitoring_interval: int = 300
    ):
        "Start monitoring a model",
        monitor_key = "{model_id}_{model_version}",

        if monitor_key in self.active_monitors:
            print("⚠️ Already monitoring model: {model_id} v{model_version}")
            return
        ...
        print("📊 Started monitoring: {model_id} v{model_version}")
    ...
    def stop_monitoring(self, model_id: str, model_version: str):
        ...
        if monitor_key in self.active_monitors:
            ...
            print("⏹️ Stopped monitoring: {model_id} v{model_version}")
    ...
    def log_prediction()
        ...
        log_id = "pred_{model_id}_{int(time.time() * 1000)}",
    ...
    def compute_metrics()
        self, model_id: str, model_version: str, time_window_hours: int = 1
        ...
        # Get prediction logs
        predictions = self._get_prediction_logs()
        ...
        # Prediction times
        prediction_times = [            p.get("prediction_time_ms", 0)
            for p in predictions
            if p.get("prediction_time_ms"
        ]
    ...
    model_id=model_id,
        ...
        :
            alerts.append(                self._create_alert()
                    ...
                    "Slow response time: {metrics.avg_prediction_time_ms:.1f}ms"
                    {}
        ...
        if ()
            metrics.drift_score
            and metrics.drift_score > self.monitoring_config["drift_threshold"]
        :
            alerts.append(                self._create_alert()
                    metrics.model_id,
                    metrics.model_version,
                    "data_drift",
                    AlertSeverity.WARNING,
                    "Data drift detected: {metrics.drift_score:.3f}",
                    {}
        ...
        if metrics.outlier_rate > self.monitoring_config["outlier_rate_threshold"]:
            alerts.append(                self._create_alert()
                    ...
                    "high_outlier_rate",
                    AlertSeverity.INFO,
                    "High outlier rate: {metrics.outlier_rate:.2%}",
                    {}
        ...
        :
            alerts.append(                self._create_alert()
                    ...
                    "low_data_quality",
                    AlertSeverity.WARNING,
                    "Low data quality score: {metrics.data_quality_score:.2f}",
                    {}
        ...
        if ()
            ...
            > self.monitoring_config["error_rate_threshold"] * 2
            or latest_metrics.data_quality_score
            < self.monitoring_config["data_quality_threshold"] * 0.5
        :
            return ModelHealth.UNHEALTHY

        # Check warning conditions
        if ()
            latest_metrics.error_rate > self.monitoring_config["error_rate_threshold"]
            or latest_metrics.drift_score
            and latest_metrics.drift_score > self.monitoring_config["drift_threshold"]
            or latest_metrics.data_quality_score
            < self.monitoring_config["data_quality_threshold"]
        :
    ...
    def get_recent_metrics()
        self, model_id: str, model_version: str, hours: int = 24
        ...
        metrics_list = [        for result in results:
            data = dict()
                zip()
                    []
                        "metric_id",
        ...
        while self.is_monitoring:
            try:
                for monitor_key, monitor_config in self.active_monitors.items():
                    ...
                    now = datetime.now()
                    if ()
                        now - monitor_config["last_check"]
                    ).total_seconds() >= monitor_config["interval"]:
                        # Compute metrics
                        ...
                        self.logger.info()
                            "Monitored {model_id} v{model_version}: {len(alerts)} alerts, health: {health.value}"
                        )
    ...
    def _calculate_confidence_distribution()
        ...
        return {            "mean": float(np.mean(confidence_array),
    ...
    def _calculate_feature_importance_drift()
        self, model_id: str, model_version: str, predictions: List[Dict[str, Any]]
    ) -> Dict[str, float]:
    ...
    def _create_alert()
        ...
        "Create a new alert",
        alert_id = "alert_{model_id}_{int(time.time() * 1000)}",
    ...
    def _save_metrics(self, metrics: ModelPerformanceMetrics):
        ...
        metric_id = ()
            "metric_{metrics.model_id}_{int(metrics.timestamp.timestamp() * 1000)}"
        )
    ...
    def _update_health_status()
        self, model_id: str, model_version: str, health: ModelHealth
    ):
