...
class ModelMonitor:
    ...
    def log_prediction(self,
        ...
        ''', (
            log_id, model_id, model_version, datetime.now(),
            json.dumps(input_features), json.dumps(prediction),
            confidence, prediction_time_ms, error_message
        ))
        
        conn.commit()
        conn.close()
    
    def compute_metrics(self, model_id: str, model_version: str, 
                       time_window_hours: int = 1) -> ModelPerformanceMetrics:
        ...
        # Prediction times
        prediction_times = [p.get('prediction_time_ms', 0) for p in predictions if p.get('prediction_time_ms')]
    ...
    def check_alerts(self, metrics: ModelPerformanceMetrics) -> List[ModelAlert]:
        ...
        if metrics.error_rate > self.monitoring_config['error_rate_threshold']:
            alerts.append(self._create_alert(
                metrics.model_id, metrics.model_version,
                "high_error_rate", AlertSeverity.CRITICAL,
                f"High error rate: {metrics.error_rate:.2%}",
                {'error_rate': metrics.error_rate, 'threshold': self.monitoring_config['error_rate_threshold']}
            ))
        ...
        if metrics.avg_prediction_time_ms > self.monitoring_config['response_time_threshold_ms']:
            alerts.append(self._create_alert(
                metrics.model_id, metrics.model_version,
                "slow_response_time", AlertSeverity.WARNING,
                f"Slow response time: {metrics.avg_prediction_time_ms:.1f}ms",
                {'avg_time_ms': metrics.avg_prediction_time_ms, 'threshold': self.monitoring_config['response_time_threshold_ms']}
            ))
        
        # Data drift alert
        if metrics.drift_score and metrics.drift_score > self.monitoring_config['drift_threshold']:
            alerts.append(self._create_alert(
                metrics.model_id, metrics.model_version,
                "data_drift", AlertSeverity.WARNING,
                f"Data drift detected: {metrics.drift_score:.3f}",
                {'drift_score': metrics.drift_score, 'threshold': self.monitoring_config['drift_threshold']}
            ))
        ...
        if metrics.outlier_rate > self.monitoring_config['outlier_rate_threshold']:
            alerts.append(self._create_alert(
                metrics.model_id, metrics.model_version,
                "high_outlier_rate", AlertSeverity.INFO,
                f"High outlier rate: {metrics.outlier_rate:.2%}",
                {'outlier_rate': metrics.outlier_rate, 'threshold': self.monitoring_config['outlier_rate_threshold']}
            ))
        ...
        if metrics.data_quality_score < self.monitoring_config['data_quality_threshold']:
            alerts.append(self._create_alert(
                metrics.model_id, metrics.model_version,
                "low_data_quality", AlertSeverity.WARNING,
                f"Low data quality score: {metrics.data_quality_score:.2f}",
                {'quality_score': metrics.data_quality_score, 'threshold': self.monitoring_config['data_quality_threshold']}
            ))
    ...
    def get_model_health(self, model_id: str, model_version: str) -> ModelHealth:
        ...
        if (latest_metrics.error_rate > self.monitoring_config['error_rate_threshold'] * 2 or
            latest_metrics.data_quality_score < self.monitoring_config['data_quality_threshold'] * 0.5):
            return ModelHealth.UNHEALTHY
        
        # Check warning conditions
        if (latest_metrics.error_rate > self.monitoring_config['error_rate_threshold'] or
            latest_metrics.drift_score and latest_metrics.drift_score > self.monitoring_config['drift_threshold'] or
            latest_metrics.data_quality_score < self.monitoring_config['data_quality_threshold']):
    ...
    def get_recent_metrics(self, model_id: str, model_version: str, hours: int = 24) -> List[ModelPerformanceMetrics]:
        ...
        for result in results:
            data = dict(zip([
                'metric_id', 'model_id', 'model_version', 'timestamp',
                'prediction_count', 'avg_prediction_time_ms', 'error_rate',
                'accuracy', 'precision', 'recall', 'f1_score', 'drift_score',
                'outlier_rate', 'confidence_distribution', 'feature_importance_drift',
                'data_quality_score'
            ], result))
            
            # Parse JSON fields
            data['confidence_distribution'] = json.loads(data['confidence_distribution']) if data['confidence_distribution'] else {}
            data['feature_importance_drift'] = json.loads(data['feature_importance_drift']) if data['feature_importance_drift'] else {}
    ...
    def get_alerts(self, model_id: str = None, model_version: str = None, 
                   resolved: bool = None, hours: int = 24) -> List[ModelAlert]:
        ...
        for result in results:
            data = dict(zip([
                'alert_id', 'model_id', 'model_version', 'alert_type',
                'severity', 'message', 'details', 'timestamp',
            ...
            try:
                for monitor_key, monitor_config in self.active_monitors.items():
                    ...
                    if (now - monitor_config['last_check']).total_seconds() >= monitor_config['interval']:
                        ...
                        self.logger.info(f"Monitored {model_id} v{model_version}: {len(alerts)} alerts, health: {health.value}")
    ...
    def _get_prediction_logs(self, model_id: str, model_version: str, start_time: datetime, end_time: datetime) -> List[Dict[str, Any]]:
        ...
        for result in results:
            data = dict(zip([
                'log_id', 'model_id', 'model_version', 'timestamp',
                'input_features', 'prediction', 'confidence',
                'prediction_time_ms', 'error_message'
            ], result))
    ...
    def _calculate_outlier_rate(self, predictions: List[Dict[str, Any]]) -> float:
        ...
        confidences = [p.get('confidence', 0.5) for p in predictions if p.get('confidence') is not None]
    ...
    def _calculate_drift_score(self, model_id: str, model_version: str, current_predictions: List[Dict[str, Any]]) -> Optional[float]:
        ...
        confidences = [p.get('confidence', 0.5) for p in current_predictions if p.get('confidence') is not None]
    ...
    def _create_alert(self, model_id: str, model_version: str, alert_type: str, 
                     severity: AlertSeverity, message: str, details: Dict[str, Any]) -> ModelAlert:
        """Create a new alert"""
    ...
    def _save_metrics(self, metrics: ModelPerformanceMetrics):
        ...
        metric_id = f"metric_{metrics.model_id}_{int(metrics.timestamp.timestamp() * 1000)}"
        ...
        ''', (
            metric_id, metrics.model_id, metrics.model_version, metrics.timestamp,
            metrics.prediction_count, metrics.avg_prediction_time_ms,
            metrics.error_rate, metrics.accuracy, metrics.precision, metrics.recall,
            metrics.f1_score, metrics.drift_score, metrics.outlier_rate,
            json.dumps(metrics.confidence_distribution),
            json.dumps(metrics.feature_importance_drift), metrics.data_quality_score
        ))
    ...
    def _save_alert(self, alert: ModelAlert):
        ...
        ''', (
            alert.alert_id, alert.model_id, alert.model_version, alert.alert_type,
            alert.severity.value, alert.message, json.dumps(alert.details),
            alert.timestamp, alert.acknowledged, alert.resolved, alert.resolution_notes
        ))
