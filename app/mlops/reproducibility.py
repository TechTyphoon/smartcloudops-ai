...
class ReproducibilityManager:
    ...
    def __init__(self, snapshots_path: str = "ml_models/reproducibility"):
    ...
    def create_snapshot(self,
        ...
        environment_variables = self._get_environment_variables() if include_env_vars else {}
    ...
    def load_snapshot(self, snapshot_id: str) -> EnvironmentSnapshot:
        ...
        columns = [
            ...
            'python_version', 'platform_info', 'packages', 'environment_variables',
            'git_info', 'system_info', 'hash_signature', 'conda_environment',
            'docker_info', 'jupyter_kernels', 'cuda_info'
        ]
        data = dict(zip(columns, result))
        
        # Parse JSON fields
        json_fields = ['platform_info', 'packages', 'environment_variables', 
                      'git_info', 'system_info', 'docker_info', 'cuda_info']
        ...
        data['jupyter_kernels'] = json.loads(data['jupyter_kernels']) if data['jupyter_kernels'] else []
    ...
    def compare_environments(self, 
                           target_snapshot_id: str,
                           current_snapshot_id: str = None) -> ReproducibilityReport:
        ...
        print(f"🔍 Comparing environments: {target_snapshot.name} vs {current_snapshot.name}")
        ...
        recommendations = self._generate_recommendations(differences, target_snapshot, current_snapshot)
    ...
    def restore_environment(self, snapshot_id: str, dry_run: bool = True) -> Dict[str, Any]:
        """Restore environment from snapshot (with dry run option)"""
        snapshot = self.load_snapshot(snapshot_id)
        
        print(f"🔄 {'Simulating' if dry_run else 'Executing'} environment restoration: {snapshot.name}")
    ...
    def list_snapshots(self) -> List[EnvironmentSnapshot]:
        ...
        cursor.execute('SELECT snapshot_id FROM environment_snapshots ORDER BY created_at DESC')
        snapshot_ids = [row[0] for row in cursor.fetchall()]
        conn.close()
        
        return [self.load_snapshot(snapshot_id) for snapshot_id in snapshot_ids]
    
    def get_reproducibility_reports(self, snapshot_id: str = None) -> List[ReproducibilityReport]:
        ...
        for result in results:
            ...
            # Parse JSON fields
            data['differences'] = json.loads(data['differences']) if data['differences'] else {}
    ...
    def _get_installed_packages(self, exclude_patterns: List[str] = None) -> Dict[str, str]:
    ...
    def _get_environment_variables(self) -> Dict[str, str]:
        ...
        safe_vars = [
            ...
            'CUDA_VISIBLE_DEVICES', 'LANG', 'LC_ALL', 'TZ'
        ]
    ...
    def _get_system_info(self) -> Dict[str, str]:
        ...
        try:
            ...
            system_info['cpu_freq_max'] = psutil.cpu_freq().max if psutil.cpu_freq() else 0
    ...
    def _analyze_differences(self, target: EnvironmentSnapshot, current: EnvironmentSnapshot) -> Dict[str, Any]:
        ...
        for key in target.platform_info:
            if key in current.platform_info and target.platform_info[key] != current.platform_info[key]:
    ...
    def _calculate_compatibility_score(self, differences: Dict[str, Any]) -> float:
        ...
        total_packages = len(differences['packages']['missing']) + len(differences['packages']['extra']) + len(differences['packages']['version_mismatch'])
        if total_packages == 0:
            ...
            package_score = max(0, 1 - (missing_penalty + extra_penalty + version_penalty))
    ...
    def _generate_recommendations(self, differences: Dict[str, Any], 
                                target: EnvironmentSnapshot, current: EnvironmentSnapshot) -> List[str]:
        ...
        if differences['packages']['extra']:
            ...
            recommendations.append(f"Consider removing {extra_count} extra packages that may cause conflicts")
        
        if differences['packages']['version_mismatch']:
            version_count = len(differences['packages']['version_mismatch'])
            recommendations.append(f"Update {version_count} packages to match target versions")
    ...
    def _determine_risk_level(self, compatibility_score: float, differences: Dict[str, Any]) -> str:
...
"""
    ...
    def _plan_python_version_change(self, snapshot: EnvironmentSnapshot) -> Dict[str, str]:
    ...
    def _plan_package_installation(self, snapshot: EnvironmentSnapshot) -> List[Dict[str, str]]:
    ...
    def _plan_package_removal(self, snapshot: EnvironmentSnapshot) -> List[Dict[str, str]]:
    ...
    def _plan_package_updates(self, snapshot: EnvironmentSnapshot) -> List[Dict[str, str]]:
        """Plan package updates"""
        current_packages = self._get_installed_packages()
        to_update = []
        
        for package, target_version in snapshot.packages.items():
            if package in current_packages and current_packages[package] != target_version:
                to_update.append({
                    'package': package,
                    'current_version': current_packages[package],
    ...
    def _save_snapshot(self, snapshot: EnvironmentSnapshot):
        ...
        ''', (
            snapshot.snapshot_id, snapshot.name, snapshot.description, snapshot.created_at,
            snapshot.created_by, snapshot.python_version, json.dumps(snapshot.platform_info),
            json.dumps(snapshot.packages), json.dumps(snapshot.environment_variables),
            json.dumps(snapshot.git_info), json.dumps(snapshot.system_info),
            snapshot.hash_signature, snapshot.conda_environment,
            json.dumps(snapshot.docker_info), json.dumps(snapshot.jupyter_kernels),
    ...
    def _save_report(self, report: ReproducibilityReport):
        ...
        ''', (
            report.report_id, report.target_snapshot_id, report.current_snapshot_id,
            report.timestamp, report.is_reproducible, json.dumps(report.differences),
            json.dumps(report.recommendations), report.risk_level, report.compatibility_score
        ))
