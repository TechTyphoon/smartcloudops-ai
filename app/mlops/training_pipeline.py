...
class TrainingPipeline:
    ...
    def create_training_config(self,
                              ...
                              created_by: str = "system") -> TrainingConfig:
        ...
        try:
            ...
            if self.experiment_tracker and job.experiment_run_id:
                try:
                    ...
                    status = ExperimentStatus.COMPLETED if job.status == JobStatus.COMPLETED else ExperimentStatus.FAILED
    ...
    def get_training_config(self, config_id: str) -> TrainingConfig:
        ...
        columns = [
            ...
            'hyperparameters', 'dataset_config', 'validation_config', 
            'training_args', 'environment', 'resource_requirements',
            'created_at', 'created_by', 'version'
        ]
        data = dict(zip(columns, result))
        
        # Parse JSON fields
        json_fields = ['hyperparameters', 'dataset_config', 'validation_config', 
                      'training_args', 'environment', 'resource_requirements']
    ...
    def get_training_job(self, job_id: str) -> TrainingJob:
        ...
        # Parse JSON fields
        json_fields = ['metrics', 'validation_results', 'logs', 'artifacts', 'resource_usage']
        for field in json_fields:
            data[field] = json.loads(data[field]) if data[field] else ({} if field in ['metrics', 'validation_results', 'resource_usage'] else [])
    ...
    def list_training_configs(self) -> List[TrainingConfig]:
        ...
        cursor.execute('SELECT config_id FROM training_configs ORDER BY created_at DESC')
        config_ids = [row[0] for row in cursor.fetchall()]
        conn.close()
        
        return [self.get_training_config(config_id) for config_id in config_ids]
    
    def list_training_jobs(self, config_id: str = None, status: JobStatus = None, limit: int = None) -> List[TrainingJob]:
    ...
    def _register_default_algorithms(self):
        ...
        def sklearn_anomaly_detection(config: TrainingConfig, dataset_info: Dict[str, Any], job: TrainingJob) -> Dict[str, Any]:
            ...
            # Prepare features
            feature_columns = config.hyperparameters.get('feature_columns', df.columns.tolist())
            ...
            if y is not None:
                ...
                X_train, X_test = train_test_split(X, test_size=0.2, random_state=job.seed or 42)
                y_train = y_test = None
            ...
            metrics['validation_accuracy'] = metrics.get('test_accuracy', 1 - test_anomaly_rate)
    ...
    def _save_training_config(self, config: TrainingConfig):
        ...
        ''', (
            config.config_id, config.name, config.description, config.algorithm,
            config.framework, json.dumps(config.hyperparameters),
            json.dumps(config.dataset_config), json.dumps(config.validation_config),
            json.dumps(config.training_args), json.dumps(config.environment),
            json.dumps(config.resource_requirements), config.created_at,
            config.created_by, config.version
        ))
    ...
    def _save_training_job(self, job: TrainingJob):
        ...
        ''', (
            job.job_id, job.config_id, job.name, job.status.value,
            job.start_time, job.end_time, job.duration_seconds,
            job.output_model_path, json.dumps(job.metrics),
            json.dumps(job.validation_results), json.dumps(job.logs),
            json.dumps(job.artifacts), job.error_message,
            json.dumps(job.resource_usage), job.experiment_run_id,
            job.git_commit, job.seed
        ))
    ...
    def _validate_training_results(self, job: TrainingJob, config: TrainingConfig) -> ValidationResult:
        """Validate training results"""
        validation_config = config.validation_config
