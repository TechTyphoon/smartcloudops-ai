...
class ErrorHandler:
    ...
    def _format_error_message(self, error_record: Dict[str, Any]) -> str:
        ...
        parts = [
            "[{error_record['error_typef']}] {error_record['error_message']}",
            "Context: {error_record['contextf']}",
            "Request: {error_record['request_info'].get(
                'method', 'N/A')} {error_record['request_info'].get('url', 'N/Af')}",
        ]

        if error_record["user_info"]:
            parts.append("User: {error_record['user_info'].get('username', 'N/A')}")
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if any(
                ...
                db_error = DatabaseError("Database operation failed: {str(e)}")
