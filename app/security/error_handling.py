...
def handle_database_errors(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if any(db_error in str(e).lower() for db_error in ["database", "sql", "connection", "timeout"]):
...
def handle_external_service_errors(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if any(net_error in str(e).lower() for net_error in ["connection", "timeout", "http", "api"]):
                service_error = ExternalServiceError(f"External service error: {str(e)}")
                error_handler.log_error(service_error, {"original_error": str(e)}, "ERROR")
