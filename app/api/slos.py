...
# Create blueprint
slos_bp = Blueprint("slos", __name__, url_prefix="/slos")
...
def get_slo_status_endpoint():
    ...
    try:
        ...
        log_business_event(
            ...
            meeting_targets=sum(
                1 for s in slo_status.values() if s.get("status") == "meeting"
            ),
        )
        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "summary": {
                        ...
                        "alert": sum(
                            1 for s in slo_status.values() if s.get("status") == "alert"
                        ),
                        "critical": sum(
                            1
                            for s in slo_status.values()
    ...
    try:
        ...
        for slo_name, status in slo_status.items():
            if "error" not in status:
                ...
                metrics.append(
                    f'slo_status{{slo="{slo_name}"}} {status_value} {timestamp}'
                )
...
def slo_health_check():
    ...
    try:
        ...
        critical_count = sum(
            1 for s in slo_status.values() if s.get("status") == "critical"
        )
