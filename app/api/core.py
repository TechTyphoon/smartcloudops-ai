...
def root():
    ...
    system_info = {
        ...
        "endpoints": {
            "status": "/api/status",
            "health": "/health",
            "mlops": "/api/mlops/" if hasattr(current_app, 'mlops_service') and current_app.mlops_service else None,
            "performance": "/api/performance/metrics" if _check_performance_available() else None
        },
        "documentation": {
            "api": "/api/docs",
            "health": "/health",
            "metrics": "/api/performance/metrics" if _check_performance_available() else None
        }
    }
...
def health():
    ...
    health_data = {
        ...
        "checks": {
            "mlops_service": hasattr(current_app, 'mlops_service') and current_app.mlops_service is not None,
            "performance_monitoring": _check_performance_available(),
...
def _check_mlops_feature(feature_name):
    ...
    return hasattr(mlops_service, feature_name) and getattr(mlops_service, feature_name) is not None
...
def _calculate_cache_hit_rate(perf_summary):
    ...
    total_hit_rate = sum(
        ...
        for cache_stats in cache_stats.values()
    )
