...
class TestMLModelDeployment:
    ...
    def test_ml_model_info_endpoint(self, base_url):
        ...
        response = requests.get(f"{base_url}/api/ml/model/info", timeout=10)
    ...
    def test_ml_model_health(self, base_url):
        ...
        response = requests.get(f"{base_url}/api/ml/model/health", timeout=10)
        assert response.status_code == 200
    ...
    def test_ml_model_metrics(self, base_url):
        ...
        response = requests.get(f"{base_url}/api/ml/model/metrics", timeout=10)
        assert response.status_code == 200
    ...
    def test_ml_model_batch_prediction(self, base_url):
        ...
        response = requests.post(
            f"{base_url}/api/ml/model/batch-predict", json=test_data, timeout=15
        )
    ...
    def test_ml_model_version_info(self, base_url):
        ...
        response = requests.get(f"{base_url}/api/ml/model/versions", timeout=10)
        assert response.status_code == 200
    ...
    def test_ml_model_retrain_trigger(self, base_url):
        ...
        response = requests.post(f"{base_url}/api/ml/model/retrain", timeout=30)
