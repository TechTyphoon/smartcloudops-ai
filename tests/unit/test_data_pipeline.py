...
class TestDataPipelineManager:
    ...
    def test_ingest_data_with_metadata(self):
        ...
        try:
            ...
            version = self.pipeline_manager.ingest_data(
                df, "metadata_test", source_metadata=metadata, tags=["metadata", "test"]
            )
    ...
    def test_get_data_version_existing(self):
        ...
        try:
            ...
            # Retrieve version
            retrieved_version = self.pipeline_manager.get_data_version(
    ...
    def test_quality_assessment_good_data(self):
        ...
        try:
            ...
            assert version.quality_status in [
    ...
    def test_get_quality_report(self):
        ...
        try:
            ...
            df = pd.DataFrame(
                {"col1": [1, 2, 3, None, 5], "col2": ["a", "b", "c", "d", "e"]}
            )
    ...
    def test_transform_data_filter(self):
        ...
        try:
            ...
            # Create source data
            df = pd.DataFrame(
                {"value": [1, 5, 10, 15, 20], "category": ["A", "B", "A", "C", "B"]}
            )
            ...
            result_version = self.pipeline_manager.transform_data(
                source_version.version_id, transformations, "filtered_dataset"
            )
    ...
    def test_transform_data_invalid_type(self):
        ...
        try:
            ...
            source_version = self.pipeline_manager.ingest_data(
                df, "invalid_transform_test"
            )

            transformations = [{"type": "invalid_transformation", "params": {}}]

            with pytest.raises(ValueError, match="Unknown transformation type"):
                self.pipeline_manager.transform_data(
                    source_version.version_id, transformations
                )
    ...
    def test_transform_nonexistent_version(self):
        ...
        with pytest.raises(ValueError, match="Data version not found"):
            self.pipeline_manager.transform_data("nonexistent_version", transformations)
