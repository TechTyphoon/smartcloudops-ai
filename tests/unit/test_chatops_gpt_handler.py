...
class TestGPTHandler:
    ...
    def test_init_openai_client_error(self):
        ...
        with patch('app.chatops.gpt_handler.OpenAI', side_effect=Exception("Connection failed")):
    ...
    @pytest.mark.parametrize("query,error_type,error_message", [
    ...
    def test_sanitize_input_invalid_inputs(self, gpt_handler, query, error_type, error_message):
    ...
    def test_process_query_response_sanitization(self, gpt_handler, valid_context):
        ...
        mock_response = Mock()
        mock_response.choices = [Mock()]
        mock_response.choices[0].message.content = "<script>alert('xss')</script>Test response"
        mock_response.usage = Mock()
        mock_response.usage.total_tokens = 150
    ...
    def test_add_context_individual_fields(self, gpt_handler, context_key, context_value):
