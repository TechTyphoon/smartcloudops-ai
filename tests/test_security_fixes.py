...
class TestInputValidation:
    ...
    def test_validate_numeric_input_security(self):
        ...
        try:
            ...
            assert "Invalid numeric input" in str(e) or "Expected numeric value" in str(
                e
            )
        ...
        for file_path in files_to_check:
            ...
            if os.path.exists(full_path):
                with open(full_path, "r") as f:
                    ...
                    assert (
                        not shell_true_found
                    ), f"Found shell=True in code (not comment) in {file_path}"
                        ...
                        if line.strip() == "except:":
                            ...
                            for j in range(i + 1, min(i + 5, len(lines))):
                                ...
                                if next_line:
                                    assert (
                                        next_line != "pass"
                                    ), f"Found bare except/pass in {file_path} at line {j}"
                                    break
    ...
    def test_secrets_module_usage(self):
        ...
        if os.path.exists(full_path):
            with open(full_path, "r") as f:
                ...
                assert (
                    "secrets.choice" in content
                ), "Should use secrets.choice for cryptographic randomness"
    ...
    def test_env_template_security_comments(self):
        ...
        if os.path.exists(full_path):
            with open(full_path, "r") as f:
                ...
                assert (
                    "IAM roles" in content
                ), "Should recommend IAM roles over access keys"
