...
class MLTrainingPipeline:
    ...
    def train_model(self, X_train: np.ndarray, params: Optional[Dict[str, Any]] = None) -> IsolationForest:
    ...
    def evaluate_model(self, model: IsolationForest, X_test: np.ndarray, scaler: StandardScaler) -> Dict[str, float]:
    ...
    def run_training_pipeline(self, data_path: str, params: Optional[Dict[str, Any]] = None) -> str:
        ...
        with self.mlflow_manager.start_run(run_name=run_name) as run:
            ...
            model_uri = self.mlflow_manager.save_model(model, self.model_name, "sklearn")
            
            # Register model
            registered_uri = self.mlflow_manager.register_model(model_uri, self.model_name)
            
            logger.info(f"Training pipeline completed. Model URI: {registered_uri}")
            return registered_uri

    def deploy_model(self, model_name: str, version: str, stage: str = "Staging") -> None:
    ...
    def compare_and_promote(self, model_name: str, new_version: str, baseline_version: str = "latest") -> bool:
        ...
        """
        logger.info(f"Comparing model {model_name} v{new_version} with v{baseline_version}")
        
        # Compare models
        comparison = self.mlflow_manager.compare_models(model_name, new_version, baseline_version)
        ...
        if should_promote:
            logger.info(f"Model {new_version} performs better. Promoting to Production.")
            self.mlflow_manager.transition_model_stage(model_name, new_version, "Production")
        else:
            logger.info(f"Model {new_version} does not perform better. Keeping in Staging.")
    ...
    def cleanup_old_models(self, model_name: str, keep_versions: int = 5) -> None:
        ...
        for version in versions[keep_versions:]:
            if version.current_stage != "Production":  # Don't delete production models
                ...
                self.mlflow_manager.delete_model_version(model_name, str(version.version))
