...
class AnomalyInferenceEngine:
    ...
    def __init__(self, model_path: str = "ml_models/models/anomaly_model.pkl", config_path: str = "ml_models/config.yaml"):
        ...
        self.cache = {}
        self.cache_duration = self.config.get('inference', {}).get('cache_duration_seconds', 300)
    ...
    def detect_anomalies(self, metrics_data: Dict) -> Tuple[bool, float, str]:
        ...
        try:
            ...
            explanation = self._explain_anomaly(metrics_data, anomaly_score, severity_score, inference_time)
    ...
    def _explain_anomaly(self, metrics_data: Dict, anomaly_score: float, severity_score: float, inference_time: float) -> str:
        ...
        try:
            ...
            if contributing_factors:
                explanation_parts.append(f"Contributing factors: {', '.join(contributing_factors)}")
            
            # Add recommendations based on severity
            recommendations = self._get_recommendations(severity_level, contributing_factors)
            if recommendations:
    ...
    def _get_recommendations(self, severity_level: str, contributing_factors: List[str]) -> str:
