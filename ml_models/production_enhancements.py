...
class ProductionAnomalyDetector:
    ...
    def extract_real_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in basic_cols:
            if col in features.columns:
                ...
                features[f"{col}_rolling_mean_5m"] = (
                    features[col].rolling(window=5, min_periods=1).mean()
                )
                features[f"{col}_rolling_mean_15m"] = (
                    features[col].rolling(window=15, min_periods=1).mean()
                )
                features[f"{col}_rolling_std_5m"] = (
                    features[col].rolling(window=5, min_periods=1).std()
                )
                features[f"{col}_rolling_max_5m"] = (
                    features[col].rolling(window=5, min_periods=1).max()
                )

                # Rate of change
                features[f"{col}_rate"] = features[col].diff().fillna(0)
                features[f"{col}_acceleration"] = (
                    features[f"{col}_rate"].diff().fillna(0)
                )

                # Percentile-based features
                features[f"{col}_percentile_75"] = (
                    features[col].rolling(window=20, min_periods=1).quantile(0.75)
                )
                features[f"{col}_percentile_25"] = (
                    features[col].rolling(window=20, min_periods=1).quantile(0.25)
                )
                features[f"{col}_iqr"] = (
                    features[f"{col}_percentile_75"] - features[f"{col}_percentile_25"]
                )

        # Cross-correlation features
        if (
            "cpu_usage_avg" in features.columns
            and "memory_usage_pct" in features.columns
        ):
            features["cpu_memory_ratio"] = features["cpu_usage_avg"] / (
                features["memory_usage_pct"] + 1e-6
            )
            features["cpu_memory_correlation"] = (
                features["cpu_usage_avg"].rolling(10).corr(features["memory_usage_pct"])
            )

        # System health indicators
        if (
            "response_time_p95" in features.columns
            and "request_rate" in features.columns
        ):
            features["throughput_efficiency"] = features["request_rate"] / (
                features["response_time_p95"] + 1e-6
            )
    ...
    def detect_anomalies_production(self, metrics: Dict) -> Dict:
        ...
        try:
            ...
            explanation = self._generate_production_explanation(
    ...
    def _generate_production_explanation(
        self, metrics: Dict, predictions: Dict, confidence: float
    ) -> str:
        ...
        if anomaly_models:
            explanation_parts.append(
                f"Anomaly detected by: {', '.join(anomaly_models)}"
            )
        ...
        if contributing_factors:
            explanation_parts.append(
                f"Contributing factors: {', '.join(set(contributing_factors))}"
            )
    ...
    def _save_models(self):
        ...
        try:
            ...
            joblib.dump(
                self.feature_columns, "ml_models/models/features_production.pkl"
            )
