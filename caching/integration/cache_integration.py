        ...
        try:
            ...
            for exp in experiments[:50]:  # Limit to 50 most recent
                ...
                if exp_id:
                    ...
                    cache_manager.set(cache_key, exp, CachePolicy(
                        ttl=7200,
        ...
        try:
            ...
            for model in models[:50]:  # Limit to 50 most recent
                ...
                if model_id:
                    ...
                    cache_manager.set(cache_key, model, CachePolicy(
                        ttl=3600,
...
def _generate_cache_recommendations(stats: Dict[str, Any]) -> List[str]:
    ...
    if hit_rate < 50:
        recommendations.append("Low cache hit rate - review cache policies and TTL settings")
    elif hit_rate < 70:
        recommendations.append("Moderate cache hit rate - consider cache warming strategies")
    
    # L1 cache recommendations
    l1_hit_rate = l1_metrics.get('hit_rate', 0)
    if l1_hit_rate < 30:
        recommendations.append("Low L1 cache hit rate - increase L1 cache size or review promotion strategy")
    
    # L2 cache recommendations
    l2_hit_rate = l2_metrics.get('hit_rate', 0)
    if l2_hit_rate < 50:
        recommendations.append("Low L2 cache hit rate - review cache policies and data access patterns")
    
    # Operation recommendations
    operation_stats = stats.get('operation_stats', {})
    error_rate = operation_stats.get('errors', 0) / max(operation_stats.get('gets', 1), 1) * 100
    
    if error_rate > 5:
        recommendations.append("High cache error rate - check Redis connectivity and system resources")
    ...
    if promotion_candidates > 10:
        recommendations.append(f"{promotion_candidates} keys eligible for L1 promotion - run cache optimization")
    
    if demotion_candidates > 20:
        recommendations.append(f"{demotion_candidates} keys eligible for L1 demotion - run cache optimization")
    
    if not recommendations:
        recommendations.append("Cache performance is optimal - no immediate recommendations")
