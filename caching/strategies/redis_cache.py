...
class RedisCache:
    ...
    def set(
        ...
        try:
            ...
            should_compress = force_compression or self._should_compress(
                serialized_data
            )
            if should_compress:
            ...
            pipe.setex(
                f"{redis_key}:meta", ttl + 300, json.dumps(metadata)
            )  # Metadata TTL slightly longer
            pipe.execute()
    ...
    def warm_cache(
        ...
        for i in range(0, total_keys, batch_size):
            ...
            if (i + batch_size) % 100 == 0:
                logger.info(
                    f"Cache warming progress: {min(i + batch_size, total_keys)}/{total_keys}"
                )
    ...
    def cache_invalidate(self, key_pattern: str):
        ...
        def decorator(func: Callable) -> Callable:
            ...
            def wrapper(*args, **kwargs):
                ...
                if deleted > 0:
                    logger.info(
                        f"Invalidated {deleted} cache entries matching pattern: {key_pattern}"
                    )
