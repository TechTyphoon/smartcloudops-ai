#!/usr/bin/env python3
"""Test script to simulate GitHub Actions workflows locally."""
...
def test_security_tools():
    ...
    # Test Bandit
    success, stdout, stderr = run_command("bandit -r app/ scripts/ -f json -o /dev/null || true")
    if not success:
    ...
    # Test Safety
    success, stdout, stderr = run_command("safety check --json --output /dev/null || true")
    if not success:
