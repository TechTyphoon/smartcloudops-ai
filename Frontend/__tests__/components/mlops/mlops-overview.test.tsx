...
describe('MLOpsOverview', () => {
  ...
  describe('Loading State', () => {
    it('displays loading skeleton when data is loading', () => {
      ...
      // Check for loading skeleton elements - look for skeleton classes
  ...
  describe('Error State', () => {
    it('displays error message when statistics fetch fails', () => {
      ...
      // Check for the AlertTriangle icon (it's an SVG, not img)
  ...
  describe('Success State', () => {
    ...
    it('displays correct statistics values', () => {
      ...
      // Check model stats - use getAllByText for multiple elements
      const modelElements = screen.getAllByText('3')
      expect(modelElements.length).toBeGreaterThan(0) // Total models appears multiple times
      ...
      // Find datasets count specifically - look for the text that comes after datasets
      const datasetsElements = screen.getAllByText('2')
      expect(datasetsElements.length).toBeGreaterThan(0) // At least one "2" should exist for datasets
    })
    ...
    it('shows active experiments badge', () => {
      ...
      // Find the badge element with the number 2
      const badgeElements = screen.getAllByText('2')
      const activeBadge = badgeElements.find(element =>
        element.closest('[class*="bg-"]') !== null
      )
    ...
    it('displays quality status distribution', () => {
      ...
      // Check for quality distribution data - look for percentage patterns
      const qualityIndicators = screen.getAllByText(/\d+\.\d+%/)
      expect(qualityIndicators.length).toBeGreaterThan(0) // Should have percentage indicators
    })
  })
  ...
  describe('Real-time Controls', () => {
    ...
    it('displays real-time controls in header', () => {
      // Set real-time to disabled for this test
      mockStore.realTimeEnabled = false
      render(<MLOpsOverview />)
    ...
    it('toggles real-time updates when live button is clicked', async () => {
      // Set real-time to disabled first to test the toggle
      mockStore.realTimeEnabled = false
      render(<MLOpsOverview />)
  ...
  describe('Accessibility', () => {
    ...
    it('has proper ARIA labels for interactive elements', () => {
      // Set real-time to disabled to test the "Start Live" button
      mockStore.realTimeEnabled = false
      render(<MLOpsOverview />)
