...
export default function RemediationPage() {
  ...
  const handleStop = (id: string) => {
    setActions((prev) => prev.map((action) => (action.id === id ? { ...action, status: "stopped" } : action)))
  }

  const handleApprove = (id: string) => {
    setActions((prev) =>
      prev.map((action) => (action.id === id ? { ...action, status: "running", progress: 0 } : action)),
    )
  }
  ...
  const handleEmergencyStop = () => {
    ...
    setActions((prev) =>
      prev.map((action) => (action.status === "running" ? { ...action, status: "stopped" } : action)),
    )
