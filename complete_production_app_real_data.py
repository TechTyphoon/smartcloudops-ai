    ...
    try:
        ...
        if os.path.exists(data_path):
            ...
            return {
                ...
                "statistics": {
                    "cpu_avg": float(df['cpu_usage_avg'].mean()) if 'cpu_usage_avg' in df.columns else 0,
                    "memory_avg": float(df['memory_usage_pct'].mean()) if 'memory_usage_pct' in df.columns else 0,
                    "disk_avg": float(df['disk_usage_pct'].mean()) if 'disk_usage_pct' in df.columns else 0
                }
            }
    ...
    try:
        ...
        if os.path.exists(health_log_path):
            ...
            return {
                ...
                "success_rate": (len(successful_checks) / (len(successful_checks) + len(failed_checks)) * 100) if (len(successful_checks) + len(failed_checks)) > 0 else 0,
                "latest_entries": logs[-10:] if logs else [],
                "endpoints_monitored": len([line for line in logs if "endpoints healthy" in line]),
    ...
    try:
        ...
        if os.path.exists(bandit_path):
            ...
            return {
                ...
                "total_issues": bandit_data.get("metrics", {}).get("_totals", {}).get("CONFIDENCE.HIGH", 0),
                "severity_breakdown": {
                    "high": bandit_data.get("metrics", {}).get("_totals", {}).get("SEVERITY.HIGH", 0),
                    "medium": bandit_data.get("metrics", {}).get("_totals", {}).get("SEVERITY.MEDIUM", 0),
                    "low": bandit_data.get("metrics", {}).get("_totals", {}).get("SEVERITY.LOW", 0)
                },
                "confidence_levels": {
                    "high": bandit_data.get("metrics", {}).get("_totals", {}).get("CONFIDENCE.HIGH", 0),
                    "medium": bandit_data.get("metrics", {}).get("_totals", {}).get("CONFIDENCE.MEDIUM", 0),
                    "low": bandit_data.get("metrics", {}).get("_totals", {}).get("CONFIDENCE.LOW", 0)
                }
            }
    except Exception as e:
...
def get_real_flask_app_stats():
    ...
    try:
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'cpu_percent', 'memory_info', 'create_time']):
            try:
                if proc.info['cmdline'] and 'complete_production_app.py' in ' '.join(proc.info['cmdline']):
                    flask_processes.append({
                        "pid": proc.info['pid'],
                        "cpu_percent": proc.info['cpu_percent'],
                        "memory_mb": round(proc.info['memory_info'].rss / (1024*1024), 2),
                        "runtime_seconds": int(time.time() - proc.info['create_time']),
                        "status": proc.status()
                    })
        ...
        return {
            ...
            "avg_cpu_percent": sum(p["cpu_percent"] for p in flask_processes) / len(flask_processes) if flask_processes else 0
        }
    except Exception as e:
...
def analyze_real_anomalies(metrics_data):
    ...
    try:
        ...
        for metric in metrics_data:
            ...
            cpu_deviation = abs(cpu_val - stats.get("cpu_avg", 50)) / stats.get("cpu_avg", 50)
            memory_deviation = abs(memory_val - stats.get("memory_avg", 50)) / stats.get("memory_avg", 50)
            ...
            if is_cpu_anomaly or is_memory_anomaly:
                anomalies.append({
                    ...
                    "severity": "high" if (cpu_deviation > 0.5 or memory_deviation > 0.5) else "medium",
...
def health():
    ...
    return jsonify({
        ...
        "uptime_seconds": current_metrics.get("system", {}).get("uptime_seconds", 0),
...
def status():
    ...
    return jsonify({
        ...
        "ml_model": {
            "status": "active",
            "training_data_records": real_ml_results.get("ml_training", {}).get("total_records", 0),
            "features_count": len(real_ml_results.get("ml_training", {}).get("features", [])),
            "data_date_range": real_ml_results.get("ml_training", {}).get("date_range", {}),
            "baseline_metrics": real_ml_results.get("ml_training", {}).get("statistics", {}),
...
def anomaly_train():
    ...
    return jsonify({
        ...
        "records_processed": real_ml_results.get("ml_training", {}).get("total_records", 0),
...
def remediation_execute():
    ...
    try:
        ...
        if action == "memory_cleanup" and not dry_run:
            ...
            execution_log.append(f"Would restart process: {target} (dry run enabled for safety)")
...
def chatops_history():
    ...
    try:
        ...
        for i, log_entry in enumerate(recent_logs[-5:]):  # Last 5 entries
            history.append({
                "id": f"real_entry_{i}",
                "timestamp": log_entry.split(' - ')[0] if ' - ' in log_entry else datetime.now().isoformat(),
...
def chatops_analyze():
    ...
    try:
        ...
        elif any(word in query_lower for word in ['security', 'scan', 'vulnerabilities']):
        ...
        return jsonify({
            ...
            "real_data_context": {
                "system_metrics": current_metrics,
                "training_records": real_ml_results.get('ml_training', {}).get('total_records', 0),
...
if __name__ == '__main__':
    ...
    logger.info(f"📊 ML Training Records: {real_ml_results.get('ml_training', {}).get('total_records', 0)}")
    logger.info(f"🔍 Health Log Entries: {real_health_data.get('total_log_entries', 0)}")  
    logger.info(f"🔒 Security Issues Found: {real_security_data.get('total_issues', 0)}")
