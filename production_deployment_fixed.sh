#!/bin/bash
# Smart CloudOps AI - Fixed Production Deployment Script
# Deploys complete application with all required endpoints

set -e

echo "🚀 Smart CloudOps AI - FIXED PRODUCTION DEPLOYMENT"
echo "Timestamp: $(date)"
echo "==============================================="

# Get current instances and their details
echo "📋 Getting current instance information..."
INSTANCES=$(aws ec2 describe-instances --region us-east-1 --query 'Reservations[*].Instances[?State.Name==`running`].[InstanceId,PublicIpAddress,Tags[?Key==`Name`].Value|[0],SecurityGroups[0].GroupId]' --output text)
...
# Parse instances
while IFS=$'\t' read -r instance_id public_ip name security_group; do
    if [[ "$name" == *"application"* ]]; then
        APP_INSTANCE_ID="$instance_id"
        APP_SERVER="$public_ip"
        APP_SECURITY_GROUP="$security_group"
    elif [[ "$name" == *"monitoring"* ]]; then
        MONITORING_INSTANCE_ID="$instance_id"
        MONITORING_SERVER="$public_ip"
        MONITORING_SECURITY_GROUP="$security_group"
    fi
done <<< "$INSTANCES"

echo "Application Server: $APP_SERVER ($APP_INSTANCE_ID)"
echo "Monitoring Server: $MONITORING_SERVER ($MONITORING_INSTANCE_ID)"

# Get my IP for security group rules
MY_IP=$(curl -s ipinfo.io/ip)
echo "My IP: $MY_IP"

# Add SSH access to security groups
echo "🔐 Configuring security groups for SSH access..."
if [ ! -z "$APP_SECURITY_GROUP" ]; then
    aws ec2 authorize-security-group-ingress --group-id "$APP_SECURITY_GROUP" --protocol tcp --port 22 --cidr "$MY_IP/32" 2>/dev/null || echo "SSH rule already exists for app"
    aws ec2 authorize-security-group-ingress --group-id "$APP_SECURITY_GROUP" --protocol tcp --port 5000 --cidr "0.0.0.0/0" 2>/dev/null || echo "Port 5000 rule already exists"
fi

if [ ! -z "$MONITORING_SECURITY_GROUP" ]; then
    aws ec2 authorize-security-group-ingress --group-id "$MONITORING_SECURITY_GROUP" --protocol tcp --port 22 --cidr "$MY_IP/32" 2>/dev/null || echo "SSH rule already exists for monitoring"
    aws ec2 authorize-security-group-ingress --group-id "$MONITORING_SECURITY_GROUP" --protocol tcp --port 3000 --cidr "0.0.0.0/0" 2>/dev/null || echo "Port 3000 rule already exists"
    aws ec2 authorize-security-group-ingress --group-id "$MONITORING_SECURITY_GROUP" --protocol tcp --port 9090 --cidr "0.0.0.0/0" 2>/dev/null || echo "Port 9090 rule already exists"
fi
...
# Create complete application deployment script
cat > /tmp/complete_app_deploy.sh << 'EOFAPP'
#!/bin/bash
exec > >(tee /var/log/smartcloudops-deploy.log) 2>&1
echo "=== Smart CloudOps AI Complete Application Deployment ==="
date

# Update system
sudo yum update -y
sudo yum install -y docker git python3 python3-pip curl htop
...
sudo mkdir -p /opt/smartcloudops
sudo chown ec2-user:ec2-user /opt/smartcloudops
cd /opt/smartcloudops

# Create complete Flask application with all required endpoints
cat > complete_app.py << 'EOFPYTHON'
#!/usr/bin/env python3
...
sudo tee /etc/systemd/system/smartcloudops.service << EOFSERVICE
[Unit]
...
sudo systemctl enable smartcloudops
sudo systemctl start smartcloudops

echo "✅ Smart CloudOps AI application deployed and started"
systemctl status smartcloudops --no-pager
EOFAPP

# Deploy to application server
echo "🚀 Deploying complete application to $APP_SERVER..."
if [ ! -z "$APP_SERVER" ]; then
    # Copy deployment script
    scp -i ~/.ssh/smartcloudops-ai-key.pem -o StrictHostKeyChecking=no /tmp/complete_app_deploy.sh ec2-user@$APP_SERVER:/tmp/ || {
        echo "❌ SCP failed, trying alternative deployment method"
        exit 1
    }
    
    # Execute deployment
    ssh -i ~/.ssh/smartcloudops-ai-key.pem -o StrictHostKeyChecking=no ec2-user@$APP_SERVER 'chmod +x /tmp/complete_app_deploy.sh && /tmp/complete_app_deploy.sh' || {
    ...
    echo "❌ No application server found"
    exit 1
fi

echo "🎉 Production deployment completed!"
echo "Application URL: http://$APP_SERVER:5000"
echo "Health Check: http://$APP_SERVER:5000/health"
