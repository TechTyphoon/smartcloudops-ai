...
def get_real_docker_stats():
    ...
    try:
        result = subprocess.run(['docker', 'stats', '--no-stream', '--format', 'table {{.Name}}\t{{.CPUPerc}}\t{{.MemUsage}}\t{{.NetIO}}'], 
...
if __name__ == "__main__":
    ...
    print(f"  Memory Usage: {real_metrics['memory']['usage_percent']}% ({real_metrics['memory']['used_gb']:.1f}GB/{real_metrics['memory']['total_gb']:.1f}GB)")
    print(f"  Disk Usage: {real_metrics['disk']['usage_percent']}% ({real_metrics['disk']['used_gb']:.1f}GB/{real_metrics['disk']['total_gb']:.1f}GB)")
    ...
    if isinstance(docker_stats, list):
        for container in docker_stats:
            print(f"  {container['name']}: CPU {container['cpu_percent']}, Memory {container['memory_usage']}")
    else:
    ...
    if flask_stats['is_running']:
        for proc in flask_stats['flask_processes']:
            print(f"  PID {proc['pid']}: CPU {proc['cpu_percent']}%, Memory {proc['memory_mb']}MB")
    else:
