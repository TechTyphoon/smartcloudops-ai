...
def main():
    ...
    print(f"   Rows: {dataset_version.row_count}, Columns: {dataset_version.column_count}")
    ...
    if models:
        ...
        for i in range(10):
            # Create sample input
            input_features = {f'feature_{j}': float(np.random.normal(0, 1)) for j in range(n_features)}
            
            # Simulate prediction (normally would use actual model)
            prediction = {"anomaly_score": float(np.random.random()), "is_anomaly": bool(np.random.random() > 0.9)}
        ...
        # Get model health
        health = model_monitor.get_model_health(latest_model['model_id'], model_metadata.version)
