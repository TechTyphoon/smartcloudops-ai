...
class PerformanceMonitor:
    ...
    def start_monitoring(self, duration_seconds: Optional[int] = None):
        ...
        print(f"🔍 Starting performance monitoring for {duration_seconds or 'unlimited'} seconds...")
    ...
    def stop_monitoring(self):
        ...
        print(f"⏹️ Performance monitoring stopped. Collected {len(self.samples)} samples.")
    ...
    def _store_sample(self, sample: PerformanceSample):
        ...
        avg_response_time = statistics.mean(sample.response_times) if sample.response_times else 0
        p95_response_time = statistics.quantiles(sample.response_times, n=20)[18] if len(sample.response_times) >= 5 else avg_response_time
        ...
        ''', (
            sample.timestamp, sample.cpu_percent, sample.memory_mb, sample.memory_percent,
            sample.disk_io_read_mb, sample.disk_io_write_mb, sample.network_sent_mb, sample.network_recv_mb,
            sample.open_files, sample.threads, avg_response_time, p95_response_time,
            sample.error_count, sample.request_count, self.start_time
        ))
    ...
    def analyze_performance(self) -> Dict[str, Any]:
        ...
        if all_response_times:
            ...
            response_percentiles = {
                ...
                'p95': statistics.quantiles(sorted_times, n=20)[18] if len(sorted_times) >= 5 else statistics.median(sorted_times),
                'p99': statistics.quantiles(sorted_times, n=100)[98] if len(sorted_times) >= 10 else statistics.median(sorted_times),
        ...
        analysis = {
            ...
            'cpu_stats': {
                ...
                'max': max(cpu_values),
                'min': min(cpu_values),
                'p95': statistics.quantiles(cpu_values, n=20)[18] if len(cpu_values) >= 5 else max(cpu_values)
            },
            'memory_stats': {
    ...
    def _identify_bottlenecks(self) -> List[str]:
        ...
        if all_response_times:
            ...
            if avg_response > 500:  # 500ms threshold
                bottlenecks.append(f"Slow response times - Average {avg_response:.1f}ms")
        
        # Database/IO bottleneck (inferred from slow responses + normal CPU)
        if all_response_times and avg_cpu < 50:
            p95_response = statistics.quantiles(sorted(all_response_times), n=20)[18] if len(all_response_times) >= 5 else avg_response
            if p95_response > 1000:  # 1 second
                bottlenecks.append("Potential database/IO bottleneck - slow responses with low CPU")
        ...
        if thread_counts:
            ...
            if avg_threads > 100:
                bottlenecks.append(f"High thread count - Average {avg_threads:.0f} threads")
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if avg_memory > 85:
            ...
            recommendations.append("Monitor memory growth and consider caching optimizations")
        ...
        if all_response_times:
            ...
            if avg_response > 1000:
                ...
                recommendations.append("Review API endpoint performance and add indices")
    ...
    def _check_threshold_violations(self) -> Dict[str, Any]:
        ...
        if all_response_times:
            p95_response = statistics.quantiles(sorted(all_response_times), n=20)[18] if len(all_response_times) >= 5 else max(all_response_times)
...
"""
        ...
        critical_issues = sum(1 for v in violations.values() if isinstance(v, dict) and v.get('exceeded', False))
        
        if critical_issues == 0:
            report += "✅ **Status**: All performance thresholds met\n\n"
        else:
            report += f"⚠️ **Status**: {critical_issues} performance threshold(s) exceeded\n\n"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SmartCloudOps AI Performance Analyzer')
    ...
    try:
        ...
        print(f"🖥️ CPU: Avg {cpu_stats.get('avg', 0):.1f}% | Peak {cpu_stats.get('max', 0):.1f}%")
        print(f"🧠 Memory: Avg {memory_stats.get('avg_percent', 0):.1f}% | Peak {memory_stats.get('peak_percent', 0):.1f}%")
        
        if response_stats:
            print(f"⚡ Response: Avg {response_stats.get('avg', 0):.1f}ms | P95 {response_stats.get('p95', 0):.1f}ms")
