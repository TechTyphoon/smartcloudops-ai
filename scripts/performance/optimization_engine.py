...
class PerformanceOptimizer:
    ...
    def run_performance_test(self, test_name: str) -> Dict[str, float]:
        ...
        try:
            ...
            cmd = [
                ...
                '--run-time', f"{self.test_config['duration']}s",
                '--csv', os.path.join(test_dir, 'results'),
                '--headless',
                '--loglevel', 'WARNING'
            ]
            ...
            for line in lines[1:]:  # Skip header
                ...
                if len(parts) >= 10 and parts[1] == 'Aggregated':
                    ...
                    if metrics['total_requests'] > 0:
                        metrics['error_rate'] = (metrics['failure_count'] / metrics['total_requests']) * 100
                    else:
                        metrics['error_rate'] = 0
                    
                    # Add percentiles if available
                    if len(parts) > 15:
                        metrics['p95_response_time'] = float(parts[15]) if parts[15] else metrics['avg_response_time']
                        metrics['p99_response_time'] = float(parts[16]) if parts[16] else metrics['avg_response_time']
    ...
    def calculate_improvement(self, before: Dict[str, float], after: Dict[str, float]) -> Dict[str, float]:
        """Calculate percentage improvement between before and after metrics"""
        improvements = {}
        
        for metric in before:
            if metric in after and before[metric] > 0:
                if metric in ['avg_response_time', 'median_response_time', 'p95_response_time', 'p99_response_time', 'error_rate']:
...
'''
            ...
            try:
                ...
                if result.success:
                    ...
                    if result.improvement_percent:
                        best_metric = max(result.improvement_percent.items(), key=lambda x: abs(x[1]))
                        print(f"   Best improvement: {best_metric[0]} by {best_metric[1]:.1f}%")
                else:
    ...
    def generate_optimization_report(self, results: List[OptimizationResult]) -> str:
        ...
        report_file = os.path.join(self.results_dir, f"optimization_report_{timestamp}.md")
...
"""
        ...
        for i, result in enumerate(results, 1):
            ...
            report += f"**Implementation Time**: {result.implementation_time:.2f} seconds  \n"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SmartCloudOps AI Performance Optimization Engine')
    parser.add_argument('--url', default='http://localhost:5000', help='Application URL')
    parser.add_argument('--duration', type=int, default=60, help='Test duration per optimization')
    parser.add_argument('--users', type=int, default=20, help='Virtual users for testing')
