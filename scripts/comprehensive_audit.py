...
class SystemAuditor:
    ...
    def security_assessment(self):
        ...
        try:
            result = subprocess.run(
                ["docker", "inspect", "--format={{.Config.User}}", "cloudops-smartcloudops-app-1"],
                capture_output=True, text=True
            )
            security_results["container_security"]["non_root_user"] = result.stdout.strip() != "root"
        except Exception as e:
    ...
    def performance_benchmarking(self):
        ...
        for test_case in ml_test_cases:
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"ML Response: {data.get('is_anomaly', 'unknown')} in {ml_time:.1f}ms")
        ...
        if ml_response_times:
            performance_results["ml_performance"] = {
                "avg_inference_time_ms": sum(ml_response_times) / len(ml_response_times),
    ...
    def scalability_analysis(self):
        ...
        containers = self.audit_results["system_info"]["containers"]["container_details"]
        cloudops_containers = [c for c in containers if "cloudops" in c.get("Names", "")]
        
        scalability_results["current_capacity"] = {
            "container_count": len(cloudops_containers),
            "memory_headroom": 100 - self.audit_results["system_info"]["system_resources"]["memory_usage"],
            "cpu_headroom": 100 - self.audit_results["system_info"]["system_resources"]["cpu_usage"],
    ...
    def resource_optimization_review(self):
        ...
        mem_usage = self.audit_results["system_info"]["system_resources"]["memory_usage"]
        optimization_results["memory_optimization"] = {
            ...
            ] if mem_usage > 60 else ["Memory usage optimal"]
        }
        ...
        optimization_results["cpu_optimization"] = {
            ...
            ] if cpu_usage > 50 else ["CPU usage optimal"]
        }
    ...
    def compliance_validation(self):
        ...
        endpoint_security = self.audit_results["security_assessment"]["endpoint_security"]
        if all(ep.get("accessible") for ep in endpoint_security.values()):
            security_score += 1
        
        # Check container security
        if self.audit_results["security_assessment"]["container_security"].get("non_root_user", False):
            security_score += 1
        
        # Data protection measures
        if self.audit_results["security_assessment"]["data_protection"]["environment_variables_secured"]:
            security_score += 1
        ...
        performance_compliant = (
            ml_perf.get("avg_inference_time_ms", 0) < 150 and
            all(perf.get("avg_response_time_ms", 0) < 100 for perf in api_perfs.values())
        )
        
        compliance_results["performance_compliance"] = {
            "ml_inference_compliant": ml_perf.get("avg_inference_time_ms", 0) < 150,
            "api_response_compliant": all(perf.get("avg_response_time_ms", 0) < 100 for perf in api_perfs.values()),
    ...
    def generate_recommendations(self):
        ...
        security_compliance = self.audit_results["compliance_check"]["security_compliance"]
        ...
        bottlenecks = self.audit_results["scalability_analysis"]["bottleneck_analysis"]["identified_bottlenecks"]
        if bottlenecks:
            recommendations.append({
    ...
    def save_audit_report(self, filename: str = None):
        ...
        report_path = Path(__file__).parent.parent / "docs" / filename.replace('.json', '.md')
    ...
    def generate_markdown_report(self) -> str:
        ...
        security_score = self.audit_results["compliance_check"]["security_compliance"]["percentage"]
        performance_compliant = self.audit_results["compliance_check"]["performance_compliance"]["overall_compliant"]
        
        overall_health = "EXCELLENT" if security_score >= 80 and performance_compliant else "GOOD" if security_score >= 60 else "NEEDS_IMPROVEMENT"
...
"""
        ...
        api_performance = self.audit_results["performance_metrics"].get("api_performance", {})
        ...
        ml_performance = self.audit_results["performance_metrics"].get("ml_performance", {})
        if ml_performance:
            report += f"""
### ML Performance
...
def main():
    ...
    try:
        ...
        # Print summary
        security_score = results["compliance_check"]["security_compliance"]["percentage"]
        performance_compliant = results["compliance_check"]["performance_compliance"]["overall_compliant"]
        
        print(f"\n🏆 AUDIT SUMMARY:")
        print(f"Security Compliance: {security_score:.1f}%")
        print(f"Performance Compliant: {'✅ YES' if performance_compliant else '⚠️ NEEDS REVIEW'}")
        print(f"Production Ready: {'✅ YES' if security_score >= 80 and performance_compliant else '⚠️ WITH IMPROVEMENTS'}")
