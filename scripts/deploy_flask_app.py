...
class FlaskAppDeployer:
    ...
    def __init__(self, instance_ip: str, key_path: str = "~/.ssh/smartcloudops-ai-key.pem"):
    ...
    def copy_files_to_instance(self) -> bool:
        ...
        try:
            ...
            self.run_ssh_command(f"mkdir -p {self.app_dir}")
            ...
            for file_path in files_to_copy:
                ...
                if source.exists():
                    ...
                    scp_cmd = [
                        ...
                        "-o", "StrictHostKeyChecking=no",
                        str(source),
                        f"{self.ssh_user}@{self.instance_ip}:{self.app_dir}/"
                    ]
    ...
    def setup_environment(self) -> bool:
        ...
        try:
            ...
            self.run_ssh_command(f"cd {self.app_dir} && python3 -m venv venv")
            
            # Install dependencies
            self.run_ssh_command(f"cd {self.app_dir} && source venv/bin/activate && pip install -r requirements-minimal.txt")
    ...
    def deploy_docker(self) -> bool:
        ...
        try:
            ...
            self.run_ssh_command(f"cd {self.app_dir} && docker-compose -f docker-compose.production.yml down", capture_output=False)
            self.run_ssh_command(f"cd {self.app_dir} && docker-compose -f docker-compose.production.yml build --no-cache", capture_output=False)
            self.run_ssh_command(f"cd {self.app_dir} && docker-compose -f docker-compose.production.yml up -d", capture_output=False)
            ...
            result = self.run_ssh_command(f"cd {self.app_dir} && docker-compose -f docker-compose.production.yml ps")
    ...
    def wait_for_service(self, timeout: int = 60) -> bool:
        ...
        print(f"⏳ Waiting for Flask service to become available (timeout: {timeout}s)...")
        
        start_time = time.time()
        while time.time() - start_time < timeout:
            try:
                response = requests.get(f"http://{self.instance_ip}:{self.app_port}/health", timeout=5)
    ...
    def run_health_checks(self) -> Dict[str, bool]:
        ...
        # Status endpoint
        try:
            response = requests.get(f"http://{self.instance_ip}:{self.app_port}/status", timeout=10)
        ...
        # Metrics endpoint
        try:
            response = requests.get(f"http://{self.instance_ip}:{self.app_port}/metrics", timeout=10)
        ...
        # ChatOps endpoint
        try:
            response = requests.get(f"http://{self.instance_ip}:{self.app_port}/chatops/status", timeout=10)
            health_results["chatops_endpoint"] = response.status_code == 200
            print(f"  ✅ ChatOps endpoint: {response.status_code}")
        except Exception as e:
            health_results["chatops_endpoint"] = False
            print(f"  ❌ ChatOps endpoint failed: {e}")
    ...
    def deploy(self) -> bool:
        ...
        print(f"📍 Target instance: {self.instance_ip}")
        ...
        try:
            ...
            print(f"🏥 Health Status: {sum(health_results.values())}/{len(health_results)} endpoints healthy")
...
def main():
    ...
    parser.add_argument("--key-path", default="~/.ssh/smartcloudops-ai-key.pem", help="SSH private key path")
