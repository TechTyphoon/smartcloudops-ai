...
class BetaTestingFramework:
    ...
    def run_endpoint_test(self, scenario_name: str, scenario: Dict[str, Any]) -> Dict[str, Any]:
                ...
                if scenario.get("content_type") == "text/plain":
                    ...
                    try:
                        ...
                        if "expected_keys" in scenario:
                            missing_keys = [key for key in scenario["expected_keys"] if key not in data]
                        ...
                        if "anomaly" in scenario_name:
                            if "data" in data and "severity_score" in data["data"]:
                                result["ml_inference_time"] = data["data"].get("explanation", "").split("Inference time: ")
                                if len(result["ml_inference_time"]) > 1:
                                    result["ml_inference_time"] = result["ml_inference_time"][1].split("ms")[0]
                        
                    except json.JSONDecodeError:
                        result["content_valid"] = False
                        result["error"] = "Invalid JSON response"
            
            logger.info(f"Test completed: {scenario['name']} - {'✅ PASS' if result['success'] else '❌ FAIL'}")
            return result
    ...
    def test_ml_performance_over_time(self, duration_minutes: int = 5) -> Dict[str, Any]:
        ...
        test_metrics = [
            {
                "cpu_usage_avg": 30.5, "cpu_usage_max": 55.2, "memory_usage_pct": 40.8,
                "disk_usage_pct": 25.1, "network_bytes_total": 712.3, "request_rate": 12.2,
                "response_time_p95": 0.18
            },
            {
                "cpu_usage_avg": 85.3, "cpu_usage_max": 95.7, "memory_usage_pct": 78.1,
                "disk_usage_pct": 65.4, "network_bytes_total": 1856.9, "request_rate": 32.8,
                "response_time_p95": 1.2
            },
            {
                "cpu_usage_avg": 15.2, "cpu_usage_max": 25.8, "memory_usage_pct": 22.5,
                "disk_usage_pct": 18.7, "network_bytes_total": 256.4, "request_rate": 4.1,
        ...
        while time.time() < end_time:
            ...
            start_time = time.time()
            try:
                response = self.session.post(f"{self.base_url}/anomaly", json={"metrics": metrics})
                ...
                if response.status_code == 200:
                    ...
                    results.append({
                        "iteration": iteration,
                        "response_time_ms": round((end_request_time - start_time) * 1000, 2),
        ...
        if results:
            ...
            analysis = {
                ...
                "std_response_time": round(statistics.stdev(response_times) if len(response_times) > 1 else 0, 2),
                "results": results
            }
        else:
    ...
    def check_container_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "healthy_containers": sum(1 for c in container_status.values() if c["healthy"])
            }
    ...
    def generate_feedback_report(self, test_results: List[Dict[str, Any]], 
...
"""
        ...
        for result in test_results:
            ...
            report += f"- **Response Time**: {result.get('response_time_ms', 'N/A')}ms\n"
...
"""
    ...
    def run_comprehensive_testing(self) -> str:
        ...
        ml_performance = self.test_ml_performance_over_time(duration_minutes=2)  # Shortened for demo
        ...
        # Generate report
        report = self.generate_feedback_report(endpoint_results, ml_performance, system_metrics)
