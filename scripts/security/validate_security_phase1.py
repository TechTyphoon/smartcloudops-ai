...
class SecurityValidator:
    ...
    def check_docker_security(self) -> List[Dict]:
        ...
        for dockerfile in dockerfiles:
            try:
                ...
                # Check for root user
                if not any(
                    "USER" in line and "root" not in line.lower() for line in lines
                ):
                    issues.append(
                        {
    ...
    def run_validation(self) -> Tuple[bool, Dict]:
        ...
        for filepath in config_files:
            if any(
                exclude in str(filepath) for exclude in ["venv", ".git", "node_modules"]
            ):
                continue
...
def main():
    ...
    if len(validator.warnings) > 0:
        ...
        for warning in validator.warnings[:5]:
            print(
                f"   - [{warning.get('severity', 'LOW')}] {warning.get('file', 'General')}: {warning['message']}"
            )
    ...
    if not passed:
        ...
        if report["issues"]:
            ...
            for issue in report["issues"]:
                if "file" in issue and "line" in issue:
                    ...
                    print(
                        f"   [{issue['severity']}] {issue.get('file', 'General')}: {issue['message']}"
                    )
        ...
        print(
            "   5. Create requirements.lock with: python scripts/security/pin_dependencies.py"
        )
