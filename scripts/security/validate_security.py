...
class SecurityValidator:
    ...
    def check_dependencies_security(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result['warnings'].append("Safety not available - install with: pip install safety")
    ...
    def check_code_security(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result['warnings'].append("Bandit not available - install with: pip install bandit")
    ...
    def check_terraform_security(self) -> Dict[str, Any]:
        ...
        if terraform_dir.exists():
            ...
            tf_files = list(terraform_dir.rglob('*.tf')) + \
                      list(terraform_dir.rglob('*.tfvars'))
    ...
    def check_ci_cd_security(self) -> Dict[str, Any]:
        ...
        if workflows_dir.exists():
            workflow_files = list(workflows_dir.rglob('*.yml')) + \
                           list(workflows_dir.rglob('*.yaml'))
        ...
        for indicator in secret_indicators:
            for file_path in self.project_root.rglob(f'*{indicator}*'):
                if file_path.is_file() and not self._is_ignored_file(file_path):
                    try:
                        content = file_path.read_text()
                        if re.search(rf'{indicator}\s*[=:]\s*["\'][^"\']+["\']', content, re.IGNORECASE):
                            issues.append(f"Potential hardcoded {indicator} in {file_path.relative_to(self.project_root)}")
                    except Exception:
                        pass
